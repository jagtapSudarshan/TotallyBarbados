...
- (void)viewDidLoad {
  ...
  menuItems = @[@"Home", @"Agenda", @"Barbados Events", @"PosterBoard", @"Stream"];
   imageItems = @[@"ic_home", @"ic_people1", @"ic_people1", @"ic_people1", @"ic_people1"];
}

-(void)viewWillAppear:(BOOL)animated{self.header.cancelButton.hidden = YES;}
-(void)viewWillDisappear:(BOOL)animated{self.header.cancelButton.hidden = NO;}
...
//  cell.imageView.image = [UIImage imageNamed:[imageItems objectAtIndex:indexPath.row]];
//  cell.textLabel.text = [menuItems objectAtIndex:indexPath.row];
//  
//  
//  return cell;
  
  LeftPanelTableViewCell *cell = (LeftPanelTableViewCell*)[tableView dequeueReusableCellWithIdentifier:simpleTableIdentifier];
  ...
  if(cell == nil) {
    ...
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.imgVw.image = [UIImage imageNamed:[imageItems objectAtIndex:indexPath.row]];
    cell.titleLbl.text = [menuItems objectAtIndex:indexPath.row];
  return cell;
}
